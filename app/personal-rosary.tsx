...
export function PersonalRosary({r,act,busy,habits=[],checks={},onLinked}:{r:Rosary;act:CommunityAction;busy:boolean;habits?:{key:string;data:{title:string;active:boolean}}[];checks?:Record<string,string>;onLinked?:()=>void}){
 ...
 const step=r.personalStep??(r.slots.filter(s=>s.done).length===5?ROSARY_STEPS.length:0),finished=step===69;
 const opening=r.opening??DEFAULT_OPENING,current=rosaryStep(step,opening),prayer=prayerFor(current,opening);
 ...
 const [draft,setDraft]=useState<RosaryOpening>(()=>step===0?{...opening,include:true,position:opening.position??'end'}:opening);
 ...
 useEffect(()=>{if(step===0)setDraft({...opening,include:true,position:opening.position??'end'});},[r.opening,step]);
 ...
 async function start(){
  if(step===0&&(draft.include!==opening.include||draft.mary!==opening.mary||draft.position!==opening.position)){
 ...
 {!active&&<><RosaryMap step={step} opening={opening}/>{finished?completion:<>{step===0&&<div className="rosary-opening"><h3>Padre nuestro y tres Avemarías</h3><p>¿Cuándo querés rezarlos?</p><div className="prayer-choices" role="group" aria-label="Cuándo rezar el Padre nuestro y las tres Avemarías"><button className="soft-button" disabled={disabled} aria-pressed={draft.position==='start'} onClick={()=>setDraft({...draft,position:'start'})}>Al inicio</button><button className="soft-button" disabled={disabled} aria-pressed={draft.position==='end'} onClick={()=>setDraft({...draft,position:'end'})}>Después de los cinco misterios</button></div>{draft.include&&<div className="prayer-choices" role="group" aria-label="Tres Avemarías"><button className="soft-button" aria-pressed={draft.mary==='standard'} onClick={()=>setDraft({...draft,mary:'standard'})}>Habituales</button><button className="soft-button" aria-pressed={draft.mary==='trinitarian'} onClick={()=>setDraft({...draft,mary:'trinitarian'})}>Hija, Madre y Esposa</button></div>}</div>}<button className="primary" disabled={disabled} onClick={start}><Cross size={22}/>{step?'Continuar rezando':'Empezar a rezar'}</button><button className="text-button" disabled={disabled} onClick={()=>setConfirm('discard')}>Descartar este rosario</button></>}{error&&<p role="alert" className="notice">{error}</p>}</>}
 ...
 <div className={"rosary-focus-body"+(!confirm&&!options&&!finished&&litany===null?" rosary-reading"+(expanded?" prayer-is-expanded":""):"")} ref={body}>

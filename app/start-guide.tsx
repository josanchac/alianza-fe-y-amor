...
export function StartCard({role,onCreate,onCouple,linked=false}:{role:string;linked?:boolean;onCreate:(habit?:Habit)=>void;onCouple:()=>void}){
 return <section className="card start-card">
  ...
  <p>Elegí un compromiso que tenga sentido para vos.</p>
...
export function QuickHelp({open,onOpenChange,onNavigate,onInstall,linked=false}:{open:boolean;linked?:boolean;onOpenChange:(open:boolean)=>void;onNavigate:(tab:string)=>void;onInstall:()=>void}){
 ...
 return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="editor-dialog help-dialog" showCloseButton={false}>
  ...
  <details><summary>¿Por dónde empiezo?</summary><p>En «Horario → Mi día», creá un compromiso y elegí en qué momento del día querés vivirlo. Cuando lo hagás, tocá su casilla. Con eso basta para empezar.</p><button className="text-button" onClick={()=>go('today')}>Ir a mi horario</button></details>
  <ScheduleGuide/><details><summary>¿Dónde anoto mi propósito?</summary><p>En «Horario → Mi mes» podés escribir el propósito que estás trabajando y revisar cómo te fue. La app organiza estas notas por mes; eso no significa que debás cambiar tu propósito cada mes.</p><p>Para precisar el propósito y su examen, seguí la pauta que trabajás con tu acompañante espiritual.</p></details>
  {linked&&<details><summary>¿Cómo usamos las 4 Rs?</summary><RsGuide/><p>Elegí una R para preparar o registrar un encuentro. La fecha, las preguntas y las notas ayudan; no hace falta llenar cada campo. Lo que guarden aquí es compartido entre ustedes.</p><button className="text-button" onClick={()=>go('couple')}>Ir a las 4 Rs</button></details>}
  {linked&&<MarriageIdealGuide/>}<details><summary>¿Y si un día no lo hago?</summary><p>Podés dejarlo sin marcar. En los tres puntos del compromiso también están «Me costó» y «Hoy no aplicaba». Una casilla vacía no se cuenta como un incumplimiento.</p><p>Si necesitás menos compromisos, abrí sus tres puntos → Editar → Más opciones y apagá «Incluir en mi horario». El historial se conserva.</p></details>
  <details><summary>¿Quién puede ver lo que escribo?</summary><p>Tu horario y tus reflexiones son privados dentro de la app. En «Mi espacio» podés dar o quitar visibilidad a tu cónyuge. Las 4 Rs pertenecen al espacio compartido. Allí también se explica el alcance de la privacidad.</p><button className="text-button" onClick={()=>go('settings')}>Revisar mi privacidad</button></details>
  <details><summary>¿Dónde veo mi avance?</summary><p>En «Horario → Historial» podés revisar semanas, meses, años o un período elegido y compararlos. Los resúmenes se forman con sus registros; no tenés que volver a escribirlos.</p><button className="text-button" onClick={()=>go('journey')}>Mirar mi camino</button></details>
  ...
  <details><summary>¿Cómo pongo recordatorios?</summary><p>En «Mi espacio», elegí una hora y tocá «Añadir al calendario». Abrí el archivo y confirmá que se agregue al calendario del celular. Los avisos son opcionales y dependen de ese calendario.</p></details>

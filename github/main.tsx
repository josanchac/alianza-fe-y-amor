...
function Login({client,setupPassword=false,emailRecoveryEnabled=false}:{client:SupabaseClient;setupPassword?:boolean;emailRecoveryEnabled?:boolean}){
  ...
  useEffect(()=>{
    ...
    const {data:{subscription}}=client.auth.onAuthStateChange((event,next)=>{
      ...
      if(event==='SIGNED_OUT'){setPassword('');setRepeat('');setMode('login');}
    });
  ...
  async function submit(e:React.FormEvent){e.preventDefault();if(busy)return;setBusy(true);setError('');setMessage('');
    try{
      ...
      if(mode==='login'){
        ...
        setPassword('');setRepeat('');setMode('login');setMessage('Contraseña guardada.');history.replaceState(null,'',new URL('./',location.href));
      }
  ...
  return <main className="auth-page"><section className="auth-card"><img className="auth-emblem" src="./emblem.png" alt="Árbol y rosario entrelazados"/><div className="brand-word">Alianza<span>FE Y AMOR</span></div><p className="auth-intro">Un camino compartido.<br/>Una entrega personal.</p><h1>{mode==='login'?'Tu espacio de fe y amor':mode==='recover'?'Recuperar mi acceso':'Elegir mi contraseña'}</h1>{mode==='password'&&session&&<div className="auth-account"><span>Tu usuario es tu correo electrónico</span><strong>{session.user.email}</strong><p>Usá este correo y tu contraseña para entrar la próxima vez.</p></div>}<form onSubmit={submit}>

...
function Login({client,setupPassword=false,emailRecoveryEnabled=false,invitationsEnabled=false}:{client:SupabaseClient;setupPassword?:boolean;emailRecoveryEnabled?:boolean;invitationsEnabled?:boolean}){
  ...
  const request=useMemo(()=>(async(init?:RequestInit)=>{
    ...
    if(pilotEnabled.current){void client.rpc('alianza_pilot_metrics',{payload:{action:'event',event:write?(error?'save_error':'save_ok'):(error?'load_error':'load_ok')}}).then(()=>{});if(!write&&performance.now()-started>3000)void client.rpc('alianza_pilot_metrics',{payload:{action:'event',event:'slow_load'}}).then(()=>{});}

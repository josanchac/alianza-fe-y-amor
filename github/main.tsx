  ...
  async function submit(e:React.FormEvent){e.preventDefault();if(busy)return;setBusy(true);setError('');setMessage('');
    try{
      if(mode==='login'){
        ...
        if(password.length<12)throw new Error('Elegí una contraseña de al menos 12 caracteres. Puede ser una frase fácil de recordar.');
        if(password!==repeat)throw new Error('Las contraseñas no coinciden.');
        const {error}=await client.auth.updateUser({password});if(error)throw new Error('No pudimos cambiarla. Solicitá un nuevo enlace o elegí otra contraseña.');

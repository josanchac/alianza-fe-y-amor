...
import {plansFor,planAt,isActiveOn,registeredCount,progressFor,type Frequency} from './schedule';
...
export function rhythmProgress(rows:RecordItem[],date:string,today:string,period:Frequency['period']){
 ...
 const values=applicable.map(h=>period==='day'?(registeredCount(checks[h.key])?1:0):(()=>{const p=progressFor(rows,h.key,date,today);return p.target?Math.min(1,p.done/p.target):null;})());

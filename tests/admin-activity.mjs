...
for(const f of migrations.filter(x=>x<'20260909143538'&&!x.endsWith('_multi_couple_isolation.sql')&&!x.endsWith('_private_admin_activity.sql')&&!x.endsWith('_metrics_auth_boundary.sql')))await db.exec(await readFile('supabase/migrations/'+f,'utf8'));

...
try{
 ...
 await as(1);await metrics({action:'event',event:'open'});assert.equal((await admin()).active30,1);
 for(let n=1;n<=4;n++){await as(n);await metrics({action:'event',event:'open'});await metrics({action:'event',event:'view_day'});await metrics({action:'event',event:'view_day'});}
 ...
 await as(2);await assert.rejects(admin,e=>e.code==='42501');assert.equal((await metrics({action:'consent',enabled:false})).enabled,true);await as(1);assert.equal((await admin()).active30,4);
 ...
 console.log('PASS four-person pilot, automatic aggregate metrics, one-person errors visible, daily deduplication, metadata-only payloads, general threshold unchanged and private-content isolation');

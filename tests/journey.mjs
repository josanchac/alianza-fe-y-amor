...
try{
 mount();await screen.findByRole('heading',{name:'Mi día'});fireEvent.mouseDown(screen.getByRole('tab',{name:'Mi espacio'}),{button:0,ctrlKey:false});fireEvent.click(screen.getByRole('button',{name:'Ajustes',exact:true}));fireEvent.click(screen.getByRole('button',{name:/Mi punto de partida/}));await screen.findByRole('heading',{name:'¿Qué te ayudaría hoy?'});assert.equal(writes.length,0);
 ...
 fail=false;fireEvent.click(screen.getByRole('button',{name:/Cuidar un compromiso/}));await screen.findByRole('heading',{name:'Tu primer compromiso'});
 ...
 fireEvent.click(document.querySelector('.month-previous>summary'));assert(screen.getByText('Propósito anterior'));assert(screen.getByText('Propósito actual'));

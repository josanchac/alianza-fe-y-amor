...
try{
 ...
 const d=await person('community-a'),e=await person('community-b');
 const group=(await rpc(d.client,'community',{action:'group_create',name:'Curso API sintético',displayName:'D',dataEpoch:1})).groups[0];
 assert.equal((await rpc(e.client,'community',{})).groups.length,0);
 await assert.rejects(()=>rpc(e.client,'community',{action:'invite_create',groupId:group.id,dataEpoch:1}),x=>x.code==='42501');
 const gi=await rpc(d.client,'community',{action:'invite_create',groupId:group.id,dataEpoch:1});
 await rpc(e.client,'community',{action:'invite_join',token:gi.token,displayName:'E',dataEpoch:1});
 assert.equal((await rpc(e.client,'community',{})).groups[0].id,group.id);
 await rpc(e.client,'community',{action:'group_leave',groupId:group.id,dataEpoch:1});
 assert.equal((await rpc(e.client,'community',{})).groups.length,0);
 assert((await make().rpc('alianza_community',{payload:{}})).error);

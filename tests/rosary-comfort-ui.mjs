...
function Harness({initial=base,checked={}}){const [r,setR]=React.useState(initial);const [checks,setChecks]=React.useState(checked);return React.createElement(PersonalRosary,{r,habits,checks,busy:false,act:async p=>{calls.push(p);if(fail)throw Error('Sin conexión');if(p.action==='rosary_step')setR({...r,personalStep:p.step,progressVersion:r.progressVersion+1,...(p.step===69?{mine:Array.from({length:5},(_,i)=>({decade:i+1,day:localDate()}))}:{})});if(p.action==='rosary_opening')setR({...r,progressVersion:r.progressVersion+1,opening:{include:p.include,mary:p.mary}});if(p.action==='rosary_today')setChecks({h:'done'});if(p.action==='rosary_discard')setR({...r,cancelled:true});return {};}});}

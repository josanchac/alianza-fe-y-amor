...
for(const f of (await readdir('supabase/migrations')).filter(f=>f<'20260909143538'&&f.endsWith('.sql')&&!f.endsWith('_multi_couple_isolation.sql')&&!f.endsWith('_private_admin_activity.sql')&&!f.endsWith('_metrics_auth_boundary.sql')).sort())await db.exec(await readFile('supabase/migrations/'+f,'utf8'));

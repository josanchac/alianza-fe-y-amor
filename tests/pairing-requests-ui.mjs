...
try{
 ...
 render(React.createElement(Journal,{dataRequest:async()=>Response.json(fixture),onSignOut(){}}));await screen.findByRole('heading',{name:'Mi día'});assert(screen.getByRole('button',{name:/Tenés una solicitud/}));assert.equal(screen.queryByRole('dialog'),null);assert.equal(screen.queryByRole('button',{name:'Pareja',exact:true}),null);

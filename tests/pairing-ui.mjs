...
try{
 ...
 fireEvent(window,new Event('focus'));fireEvent.click(screen.getByRole('button',{name:'Pareja',exact:true}));await screen.findByRole('tab',{name:'En pareja'});fireEvent.mouseDown(screen.getByRole('tab',{name:'En pareja'}),{button:0,ctrlKey:false});await screen.findByRole('heading',{name:'En pareja'});
